...
export const TextInputField = ({
  ...
  return <Fragment>
      ...
      <TextInput autoCapitalize="none" style={[styles.textInput, textInputStyle]} placeholderTextColor={Color.steel} underlineColorAndroid={"transparent"} {...props} />
...
export const Button = ({
  ...
  return <TouchableOpacity style={[styles.viewStyle, viewStyle]} onPress={onPress} disabled={loading}>
      {loading ? <ActivityIndicator color={loadingColor || Color.white} style={loadingStyle} /> : <Text style={[styles.textStyle, textStyle]}>{title}</Text>}
    </TouchableOpacity>;
};
/**
...
const SocialButtons = ({
  ...
  return <Fragment>
    ...
    <Button title="Signin with Facebook" viewStyle={styles.facebookButton} textStyle={{
      color: Color.white
    }} loading={loading} onPress={onFacebookConnect} />
    <GoogleSigninButton onPress={onGoogleConnect} size={GoogleSigninButton.Size.Wide} color={GoogleSigninButton.Color.Dark} disabled={loading} style={styles.googleLoginButton} />
    {(Platform.OS === "ios" || appleAuthAndroid.isSupported) && <AppleButton onPress={onAppleConnect} buttonStyle={AppleButton.Style.WHITE_OUTLINE} buttonType={AppleButton.Type.SIGN_IN} style={styles.appleButton} />}
  </Fragment>;
};
/**
...
const onFacebookConnect = async (dispatch, navigation, setErrorResponse) => {
  ...
  try {
    ...
    if (!fbResult.isCancelled) {
      ...
      const data = await AccessToken.getCurrentAccessToken(); // This action dispatches the api for facebook login which takes FB accessToken in params
      ...
      dispatch(facebookLogin({
        ...
        if (res.key) {
          ...
          Alert.alert("SignIn Success", "You are Logged In Successfully with Facebook Account."); // This navigates the user to the home screen of the app
...
const onGoogleConnect = async (dispatch, navigation, setErrorResponse, GOOGLE_WEB_CLIENT_ID, GOOGLE_IOS_CLIENT_ID) => {
  ...
  if (!errors.length) {
    ...
    try {
      ...
      await GoogleSignin.hasPlayServices(); // Initializes the Overlay

      await GoogleSignin.signIn(); // Fetches the token

      const tokens = await GoogleSignin.getTokens(); // This action dispatches the api for google login which takes accessToken in params
      ...
      dispatch(googleLogin({
        ...
        if (res.key) {
          ...
          Alert.alert("SignIn Success", "You are Logged In Successfully with Google Account.");
          navigation.navigate(HOME_SCREEN_NAME);
        }
      }).catch(err => {
        // Error message will displayed if something goes wrong in authorization
...
const onAppleConnect = async (dispatch, navigation, setErrorResponse, APPLE_SERVICE_ID, APPLE_REDIRECT_CALLBACK) => {
  try {
    ...
    if (result.identityToken && result.authorizationCode) {
      ...
      dispatch(appleLogin({
        ...
        if (res.key) {
          // Success message will be displayed once the authorization is successfull and user is successfully logged in
          Alert.alert("SignIn Success", "You are Logged In Successfully with Apple Account.");
          navigation.navigate(HOME_SCREEN_NAME);
        }
      }).catch(error => {
        // Error message will displayed if something goes wrong in authorization
...
export const SignupTab = ({
  ...
  const [confirmPassword, setConfirmPassword] = useState(""); // Specific validations for email and password
  ...
  const dispatch = useDispatch(); // This function empties the text inputs once called
  ...
  return <KeyboardAvoidingView>
      <View style={_styles.QxupPaLm}>
        <TextInputField keyboardType="email-address" label="Email address" placeholder="Email address" onChangeText={handleEmailChange} value={email} error={validationError.email} />
        <TextInputField label="Password" placeholder="Password" secureTextEntry={true} onChangeText={handlePasswordChange} value={password} error={validationError.password} />
        <TextInputField label="Confirm Password" placeholder="Confirm Password" secureTextEntry={true} onChangeText={handleConfirmPasswordChange} value={confirmPassword} />
      </View>
      {apiError.map((value, index) => <View key={index}>
                <Text style={[styles.error, _styles.MYgGOmno]}>{value[Object.keys(value)[index]]?.toString()}</Text>
              </View>)}
      ...
      <SocialButtons loading={api.loading === "pending"} onFacebookConnect={() => {
      setErrorResponse([]);
...
export const SignInTab = ({
  ...
  const onSigninPress = async () => {
    setApiError([]);
    setErrorResponse([]); // Error message will be displayed if user has not entered a valid email
  ...
  return <KeyboardAvoidingView>
      <View style={_styles.YDYAMxhC}>
        <TextInputField keyboardType="email-address" label="Email address" placeholder="Email address" onChangeText={handleEmailChange} value={email} error={validationError.email} />
        <TextInputField label="Password" placeholder="Password" secureTextEntry={true} onChangeText={handlePasswordChange} value={password} error={validationError.password} />
      </View>
      {apiError.map((value, index) => <View key={index}>
                <Text style={[styles.error, _styles.jlHtRZGP]}>{value[Object.keys(value)[index]]?.toString()}</Text>
              </View>)}
