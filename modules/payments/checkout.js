...
export const CheckoutScreen = params => {
  ...
  const {
    ...
    onShippingMethodSelected: shippingMethod => {
      __DEV__ && console.log("shippingMethod", shippingMethod); // Update cart summary based on selected shipping method.
    },
    onShippingContactSelected: shippingContact => {
      __DEV__ && console.log("shippingContact", shippingContact); // Make modifications to cart here e.g. adding tax.
      // handler(cart);
    }
  });
  ...
  return <View>
      <View style={_styles.CjUVODWn}>
        <Text style={_styles.ouJsacpM}>Amount</Text>
        <TextInput placeholder={"Enter Amount"} value={value.amount} onChangeText={text => setValue({ ...value,
        amount: text
      })} style={styles.inputField}></TextInput>
      </View>
      <View style={_styles.PYHXiAVd}>
        <TouchableOpacity style={[styles.button, styles.payNow]} onPress={openPaymentSheet}>
          <Text style={styles.buttonText}>Pay Now</Text>
        </TouchableOpacity>

        {Platform.OS === "android" && enableGooglePay && <GooglePayButton disabled={!gPayinitialized || loading} style={[styles.payButton, {
        width: deviceWidth / 2.5,
        height: 52,
        marginTop: 5
      }]} type="pay" onPress={payGoogle} />}

        {enableApplePay && isApplePaySupported && <ApplePayButton onPress={payApple} type="plain" borderRadius={4} style={styles.payButton} />}
      </View>
    </View>;
};
